...
export default function FontSettingsButtons() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedFont = window.localStorage.getItem('userFontFamily') as FontFamily | null
      const savedSize = window.localStorage.getItem('userTextSize') as TextSize | null
    ...
    const unsubscribe = fontSettings.subscribe(({ fontFamily: newFont, textSize: newSize, lineHeight }) => {
  ...
  return (
    <div className="font-settings-container" id="font-settings">
      <span
        className="hidden lg:flex items-center text-lg font-light text-secondary-white mb-4"
